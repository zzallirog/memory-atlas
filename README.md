...
- **2.4.0** (2026-07-09) — public-release pass. All RU comments/docstrings/CLI help translated to EN (4 sequential Sonnet chunks, i18n UI keys untouched — self-test guards them). Generic defaults: `--src` autodetect = any non-empty `~/.claude/projects/*/memory` (most .md files wins) or `$ATLAS_SRC`; data home = `$ATLAS_HOME` → legacy `~/.mac-claw/atlas` (if present) → XDG cache / `%LOCALAPPDATA%`; UI language auto-detects from locale (ru/uk → ru, else en), the in-page toggle still wins after first run. Windows toolchain: `installer/build_pyz.py` (stdlib zipapp, replaces bash-only builder), utf-8-forced git subprocess, `os.startfile` open path; CI matrix (ubuntu/macos/windows × py3.8/3.12) runs self-test + demo + pyz build. Pins finish the graph-lab PinStack bucket: cards carry a 2-line note preview and **capture the view at pin time** (node-anchored zoom; click the card = restore, overrides cam=stay as explicit intent).
