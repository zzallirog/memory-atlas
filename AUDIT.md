...
**Свип 07-09 (эта ветка, третий коммит):** дополнительно к помеченным ниже ✅-фиксам закрыты одним заходом **B5** (vecs живут без semantic-cross; тест + live-прогон: vecs в DATA, 0 semantic-рёбер) · **B6** (browse-wikilinks: зеркало WIKILINK_RE+norm, alias-текст показывается; замер: dmz_canon 9 wl / 0 dead) · **B9** (applyView: saveCfg + guard старых видов) · **B13** (cfg `v:2`, магия TS===1 умерла) · **B14** (мёртвый селектор) · **B15** (pins-сватчи на colorMode change; тот же класс закрыт и в dash-клике зоны) · **B16** · **B17** · **B18** · **G6** (квотинг src). **B24 (новый, пойман playwright-пассом):** showTab не закрывал sheet-оверлеи — settings висел поверх browse; починен (смена вкладки закрывает help/regen/settings). Осталось НЕпочиненным из списка: **B2** (после rebase), **B3** (UX-решение, S4), **B8** (XSS href — рядом с B6-зоной, чинить в build), **B19** (S3-дизайн matchSet), **B20–B22** (решения глазами/доки).
...
**G3 ⚠️ ПЕРЕОЦЕНЁН 07-09 — мой изначальный фикс был бы РЕГРЕССИЕЙ.** emb-cache общий для ВСЕХ корпусов (--src memory · bastra · чужие вульты): «прунить до ключей текущего прогона» = каждый запуск одного корпуса убивает кэш остальных → вечный thrash + полный re-embed. Правильно: per-src неймспейс в кэше ИЛИ LRU с таймстампами — решение build-сессии, НЕ однострочник. (Изначальная строка аудита — образец OVERCLAIM: рекомендация без прогона второго юзкейса.) Исходно: emb-cache растёт вечно (:406–445), sha-ключи старых версий тел не выселяются.
...
**S11 · Программные сеттеры не фирят change-листенеры.** `$('colorMode').value = x` обновляет UI, но зависимые рендеры (legend/pins/legend-isolate) молчат — класс B15 воспроизвёлся в dash-клике через час после фикса B15. Кандидат: хелпер `setMode(id, v)` = value + saveCfg + пере-рендер зависимых; все программные точки (presets, views, dash, ?state=) через него.

**S12 · Глифы кнопок = юникод-рулетка шрифтов.** ⛭ ▦ ↧ ↥ ⧉ ⌖ — между хостами/шрифтами рендер гуляет (Mac эмодзифицирует, Linux-шрифт может отдать tofu). После синка на Mac — проверить глазами; при tofu → текст-лейблы или inline-SVG. Уже видно на Арче: ⚙ (rebuild) и ⛭ (settings) в баре — два почти одинаковых гира рядом, семантика неразличима.

**S13 · Новая поверхность = дизайн-чеклист ДО пуша, smoke ловит только JS.** `data-ready` + console-clean ≠ «кнопки сидят в дизайне». Чеклист: все кнопки одного ряда — одна высота/базлайн · sheet-оверлеи закрываются при смене вкладки · наслоения absolute-элементов (keys ↔ minimap!) · узкий вьюпорт (<1100px CSS бар ломается: fit в две строки) · глифы. Этот пасс поймал B24 и двугировую пару — ПОСЛЕ того как юзер спросил.

**S14 · MCP-скриншоты: dpr=2 режет вьюпорт вдвое.** resize(1728) → innerWidth 864 CSS — все layout-выводы со скриншотов сначала сверять с `window.innerWidth`, иначе «max-width сломан» оказывается кривым замером (кейс этого пасса — сам чуть не завёл ложный баг).

## 12. Post-mortem сессии 07-08/09 — мои проёбы (калибровка для билдера)

1. **G3-рекомендация была бы регрессией** — прун общего кэша по текущему корпусу убивает кэш других. Поймал сам на свипе, но В АУДИТЕ строка сутки лежала как «фикс — одна строка». Мода: OVERCLAIM (рекомендация без прогона второго юзкейса).
2. **Дизайн-пасс не входил в мой verify** — шипнул ⛭/dash-кнопки двумя коммитами без глазного прохода по поверхностям; юзер ткнул. Функциональная верификация ≠ дизайн-верификация (→ S13). Улов пасса: B24, двугировая пара, «данные:» дубль под заголовком ДАННЫЕ в ⛭, бар <1100px, keys-под-минимапой (старый, но мой TS-зум усугубил).
3. **B15-класс воспроизвёл сам** в dash-клике зоны сразу после починки B15 (→ S11).
4. **Тест B5 упал первым прогоном** — писал по памяти о фикстуре, не по её телу (label/desc отсутствовали). WRONG-SURFACE в миниатюре.
5. Гигиена: нумерация §11-перед-§10 в первой версии AUDIT · Edit-без-Read на cp-нутом файле · скриншоты в $HOME (вне whitelist) · fish-heredoc в первый ssh · фоновый http.server без cleanup-плана (двойной pkill с exit 144, mv за ним не выполнился — составные команды после kill не жильцы).
6. **VERSION не бампнут** — статус-бар говорит «v2.0.0» про ветку с dash/settings/фиксами. Осознанно (бамп = merge-конфликт с Mac), но в проде это ложь версии: бампнуть при merge в main.

## 13. Дизайн-хвосты (юзер дизайнит сам — только список)

- Бар: ⚙/⛭ пара неразличима · fit оборачивается на узких · keys-строка наслаивается на минимапу · статус-карточка громоздкая (полный src-путь, 5 строк на узких).
- ⛭: строка «данные: … назад» под заголовком «ДАННЫЕ» — дубль слова.
- Dash: пустая правая колонка при 8 карточках на широких (auto-fit) — сетку можно уплотнить; hist-подписи 9px на пределе.
- Browse: Catppuccin-остров внутри glass-атласа — намеренный вербатим, но viewtabs/hint-card поверх него выглядят чужими.
- Глифы → S12.

## 14. Открытые вопросы билдеру (решить в build-сессии)
*(нумерация: §11 pre-mortem дополнен S11–S14 выше; эта секция была §11 — второй раз ловлю себя на вставке-перед, см. post-mortem п.5)*
