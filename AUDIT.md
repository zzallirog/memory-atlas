...
**B1 ✅ FIXED (эта ветка) — замер: orphan 84 → 3 на Арч-корпусе (81 ложный = 96% шума); тест `test_orphan_respects_feedback_index`. · generator:176 · `orphan` перевирает feedback-зону.** `orphan = not in_light and not in_heavy and not top` — нота, индексированная ТОЛЬКО в FEEDBACK_INDEX.md, получает `zone=feedback` (фикс 07-06), но `orphan=True` — fb_idx в флаге не участвует. Browse-тайл «orphan» и бейджи перевирают на ~80 нот. Тот же класс бага, что «81 unindexed» до 07-06 — недочинен. Фикс: `and stem not in fb_idx`. Тест на это добавить.

**B2 ⏳ ПОСЛЕ REBASE (S1: строка в node-loop вплотную к tile-LOD 2461–2467 — живая зона Mac; чинить на её коде) · template:2471 · pin-контракт ломается под поиском.** `globalAlpha = dimmed(n) ? 0.10 : pinDimmed(n)…` — dimmed() включает query/tags/isolate/maxN и бьёт ПЕРВЫМ: запиненная нода, не матчащая поиск, гаснет до 0.10. Контракт «pins остаются яркими» (context delivery) нарушен всеми фильтрами. Фикс: pinned (и selected) исключить из dimmed либо порядок веток.
...
**B7 ✅ FIXED (эта ветка) — замер: индекс-призраки 1 → 0 (`feedback_index`); тест `test_index_refs_no_ghost`. · generator:202–221 · ссылки на индекс-файлы рождают ghost-долги.** MEMORY.md/MEMORY-work.md/FEEDBACK_INDEX исключены из нод, но `[[MEMORY-work]]`/`[x](MEMORY.md)` из тел нот резолвятся в ghosts «memory_work» и т.п. — ложный «стоит написать». Фикс: стемы INDEX_FILES → skip (ни ребра, ни ghost'а), тест.
...
## 10. Pre-mortem — где споткнёмся (07-08, вторая итерация; ranked по боли)

**S1 · Merge с Mac — не «зоны», а сосединые строки одного файла.** Обе стороны лезут в одни точки врезки: esc-каскад (:1417), saveCfg-схему, `#viewtabs` (dash = третья вкладка, её фиксы = те же блоки). Line-refs аудита СТУХНУТ после её пуша. Митигация: якорить по именам функций/сниппетам (в аудите есть), после rebase перепроверить каждую B-строку грепом, не номером. B2 уже реклассифицирован по этой причине.

**S2 · `?state=` кросс-хост сломан by-design, если хранить transform.** Сырые x/y/k бессмысленны на другом корпусе (288 нот Арч vs ~330 Mac — другие позиции, другой sim). Дизайн ДО кода: state = {node-id центра, k, фильтры, layout}, камера восстанавливается через frameEgo/frameSet. Иначе фича «шарить вид» тихо шарит мусор.

**S3 · B19 (unified search) — perf-ловушка.** `matches()` зовётся per-node per-frame из dimmed(); впихнуть туда body-scan = регресс хуже бага. Правильно: query → precomputed Set(id) на input-событии; dimmed читает Set. Заодно убирает существующий concat-per-frame.

**S4 · B3 (preset-откат) — UX-решение, не код.** «Когда пресет кончается?» — события нет. Снапшот/автооткат может взбесить (включил слой пресетом, понравился — reload съел). Решить до стройки: пресет = session-only override + toast «зафиксировать: сохрани как view», или явная кнопка. Не изобретать полумеры в коде.

**S5 · STATE-слой (x20 №1) — данных НОЛЬ.** vault-state baseline: 0 state-тегов на 286 нот (теги появятся после ревью 07-14). Рендер до тегирования = мёртвая фича. Порядок: парсер-ключ врезать заранее (дёшево, G2 всё равно трогает парсер), рендер+тайл — после появления тегов.

**S6 · Fire-log heat (x20 №2) — это проект, не тайл.** fire-log пишет ЯДРА, не ноты; маппинга fire→note-id не существует (bastra-heat жил на bigram-матчере арки). Нужен свой measurement-design + F9-урок: разреженный heat = «тронуто/нет», не градиент. В v2.1 не тащить, отдельная итерация.

**S7 · Общий out-путь = тихая дивергенция.** `~/.mac-claw/atlas/memory-atlas.html`: Mac прислал свой (07-08 22:04, разовая копия — Syncthing дир НЕ трогает, проверено), Арч-реген его перезапишет. Не поломка (каждый хост = свой корпус), но «открыл старую вкладку — смотрю чужой корпус». Решение build-сессии: host-суффикс в имени или принять и задокументировать.

**S8 · Профиль-import обязан пропускать неизвестные ключи.** Mac докидывает свои ручки прямо сейчас → строгий парс профиля = профили ломаются между версиями template. Merge-семантика: known keys мержим, unknown — переносим as-is, stale id-refs считаем (паттерн overlayStale).

**S9 · Координация веток.** Mac-сессия НЕ знает про `arch-audit-v2`. Риск низкий: её баги — template, мои фиксы — генератор (разные файлы). Но перед merge в main — `git fetch` + перечитать её дифф целиком (verify the change, not the opinion).

**S10 · B20 живой: `rail` коллизия (glados/RAIL vs bastra/RAIL) на Арч-корпусе, n=1.** Dash-тайл «здоровье данных» получает реальный сигнал с первого дня — генератору эмитить collisions в DATA (сейчас только stderr-warn, клиент слеп).

## 11. Открытые вопросы билдеру (решить в build-сессии)
